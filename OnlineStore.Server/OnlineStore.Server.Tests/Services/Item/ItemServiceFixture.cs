...
namespace OnlineStore.Server.Tests.Services.Item
{
    public class ItemServiceFixture
    {
        ...
        public Mock<IItemRepository> CreateMockRepository()
        {
            ...
            mockRepository.Setup(x => x.Create(It.Is<ItemRequest>(x => x.Code == ItemCode_Exists))).ReturnsAsync(ItemGuid_Exists);

            //update

            mockRepository.Setup(x => x.Update(ItemGuid_Exists, It.Is<ItemRequest>(x => x.Name == ItemName_Exists
                                                                                     || x.Code == ItemCode_Exists
                                                                                     || x.Price == ItemPrice_Exists
                                                                                     || x.Category == ItemCategory_Exists)))
                          .ReturnsAsync(true);

            mockRepository.Setup(x => x.Update(Guid_Unexists, It.IsAny<ItemRequest>())).ReturnsAsync(false);

            //delete

            mockRepository.SetupSequence(x => x.Delete(ItemGuid_Exists)).ReturnsAsync(true).ReturnsAsync(false);

            mockRepository.Setup(x => x.Delete(Guid_Unexists)).ReturnsAsync(false);

            //get

            mockRepository.Setup(x => x.GetOneByCriteria(It.Is<ItemFilterCriteria>(x => x.Id == ItemGuid_Exists
            ...
            mockRepository.Setup(x => x.GetAll())
                          .ReturnsAsync(() => new ResponseList<ItemResponse>()
                          {
                              TotalCount = 1,
                              Responses = [new ItemResponse()
                              {
                                  Id = ItemGuid_Exists,
                                  Code = ItemCode_Exists,
                                  Name = ItemName_Exists,
            ...
            mockRepository.Setup(x => x.GetAllByCriteria(It.Is<ItemFilterCriteria>(x => (x.Id == null && x.Name == null && x.Code == null && x.Category == null)
                                                                                     || x.Id == ItemGuid_Exists
                                                                                     || x.Name == ItemName_Exists
                                                                                     || x.Code == ItemCode_Exists
                                                                                     || x.Category == ItemCategory_Exists)))
                          .ReturnsAsync(() => new ResponseList<ItemResponse>()
