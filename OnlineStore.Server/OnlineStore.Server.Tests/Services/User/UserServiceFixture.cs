...
namespace OnlineStore.Server.Tests.Services.User
{
    public class UserServiceFixture
    {
        ...
        public Mock<IUserRepository> CreateUserMockRepository()
        {
            ...
            mockRepository.Setup(x => x.Authenticate(It.Is<UserCredentialsRequest>(x => x.Username == Username_Exists[0])))
            ...
            mockRepository.Setup(x => x.Authenticate(It.Is<UserCredentialsRequest>(x => x.Username == Username_Exists[1])))
                .ReturnsAsync(new LoginResponse
                {
                    CustomerId = Guid.NewGuid(),
            ...
            mockRepository.Setup(x => x.Authenticate(It.Is<UserCredentialsRequest>(x => !Username_Exists.Contains(x.Username))))
                .ReturnsAsync(() => null);

            //update

            mockRepository.Setup(x => x.Update(Username_Exists[0], It.Is<UserRequest>(x => x.Username != Username_Unexists)))
                .ReturnsAsync(true);

            mockRepository.Setup(x => x.Update(Username_Exists[1], It.Is<UserRequest>(x => x.Username != Username_Unexists)))
                .ReturnsAsync(true);

            mockRepository.Setup(x => x.Update(It.Is<string>(x => !Username_Exists.Contains(x)), It.IsAny<UserRequest>()))
                .ReturnsAsync(false);

            //delete

            mockRepository.SetupSequence(x => x.Delete(Username_Exists[0])).ReturnsAsync(true).ReturnsAsync(false);

            mockRepository.SetupSequence(x => x.Delete(Username_Exists[1])).ReturnsAsync(true).ReturnsAsync(false);

            mockRepository.Setup(x => x.Delete(Username_Unexists)).ReturnsAsync(false);

            // get

            mockRepository.Setup(x => x.GetAll())
                          .ReturnsAsync(() => new ResponseList<UserResponse>()
                          {
                              TotalCount = 2,
                              Responses = [
                                  new UserResponse()
