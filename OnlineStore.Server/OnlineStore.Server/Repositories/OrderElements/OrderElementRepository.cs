...
using OnlineStore.Server.Mapping.OrderElements;
...
namespace OnlineStore.Server.Repositories.OrderElements
{
    public class OrderElementRepository(OnlineStoreDbContext context) : IOrderElementRepository
    {
        ...
        public async Task<Guid?> Create(OrderElementRequest orderElement)
        {
            ...
            OrderElement? orderElementEntity = await _context.OrderElements.FirstOrDefaultAsync(x => x.OrderId == orderElement.OrderId 
        ...
        {
            OrderElement? orderElementEntity = await _context.OrderElements.FirstOrDefaultAsync(x => x.Id == id);
